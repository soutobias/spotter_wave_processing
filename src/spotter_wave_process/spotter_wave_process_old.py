...
class SpotterProcess:
    ...
    def __init__(
        ...
        self.model_sources = model_sources or ["gfs"]
        self.era5_path = era5_path or f"{os.getcwd()}/data/era5_cropped.nc"
    ...
    def download_spotter_data(self):
        ...
        self.spotter_ds = xr.open_dataset(s3_uri, engine="zarr")
    ...
    def download_gfs_data(self):
        ...
        dss = []
        for year in years:
            url = f"https://erddap.aoml.noaa.gov/hdb/erddap/griddap/WaveWatch_{year}"
    ...
    def plot_single_frame(self, curr_time, model_name, model_var):
        curr_time = pd.Timestamp(curr_time).tz_convert("UTC")
        ...
        if model_var in ["Thgt", "swh"]:
            ...
            model_min = 0
            model_max = 8
            model_label = f"{display_name} significant wave height [m]"
            error_label = "Significant wave height difference [m]"
        else:
            cbar_min = -3
            cbar_max = 3
            model_min = 0
            model_max = 17
            model_label = f"{display_name} peak wave period [s]"
            error_label = "Peak period difference [s]"
        ...
        fig.suptitle(f"{display_name} - Spotter: hour {analysis_hour}, {model_var}")
